...
var file = path.join(__dirname, 'cartoon.json'); 
...
function fileWrite(data) {
  fs.open("cartoon2.json", "w+", function(err, fd) {
...
async function getTagMsg(data) {
    ...
    for (let doc of Object.keys(data)) {
      i++
      if(commonData[doc].videoUrl.length === 0) {
        console.log(commonData[doc].title);
