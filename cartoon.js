...
let listUrl = 'http://v.d9y.net/dongman.php?m=/dongman/list.php?cat=all&page='
let testObj = {}
...
function file(data) {
  fs.open("cartoon.json", "w+", function (err, fd) {
    if (err) {
      console.error(err)
    }
    ...
    .end((err, res) => {
      ...
      $('#dianshijua .panel').each(i => {
        ...
        el.find('.num-tab-main>a').each(z => {
          obj.url.push({
            num: el.find('.num-tab-main>a').eq(z).attr('data-num'),
